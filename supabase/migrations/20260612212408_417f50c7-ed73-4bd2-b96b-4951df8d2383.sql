ALTER TABLE public.propostas ADD COLUMN IF NOT EXISTS prazo_entrega_dias integer;
...
BEGIN
  ...
  UPDATE propostas
     SET valor_proposta = p_valor,
         ...
         prazo_entrega_dias = COALESCE(p_prazo_dias, prazo_entrega_dias),
         updated_at = now()
  ...
  PERFORM log_audit_event(
    ...
    'Consultor ajustou valor/horas/prazo da proposta pré-aprovada',
    jsonb_build_object('valor', v_old_valor, 'horas', v_old_horas),
    jsonb_build_object('valor', p_valor, 'horas', COALESCE(p_horas, v_old_horas), 'prazo_dias', p_prazo_dias, 'motivo', p_motivo),
    'info'
  );
...
BEGIN
  ...
  PERFORM log_audit_event(
    ...
    jsonb_build_object('valor', p_valor, 'horas', p_horas, 'prazo_dias', p_prazo_dias, 'justificativa', p_justificativa, 'status', 'contraproposta_consultor'),
    'warning'
  );
